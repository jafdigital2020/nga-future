@extends('layouts.hrmaster') @section('title', 'User Earning')
...
<div class="content container-fluid">
    ...
    <div class="page-header">
        <div class="row">
            <div class="col">
                <h3 class="page-title">User Earning</h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('hr/dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ url('hr/earning/user') }}">User Earning</a></li>
                </ul>
            </div>
            <div class="col-auto float-right ml-auto">
                <a href="#" class="btn add-btn m-r-5" data-toggle="modal" data-target="#assign_earning"> Assign
                    Earnings</a>
            </div>
        </div>
    </div>
    ...
    <!-- Content Starts -->
    <!-- Search Filter -->
    <form action="{{ route('hr.userEarningsIndex') }}" method="GET">
            ...
            <div class="col-sm-6 col-md-3">
                <div class="form-group form-focus select-focus">
                    <select class="select floating" name="earning">
                        <option value="" disabled selected>Select Earning</option>
                        @foreach ($earnings as $earn)
                        <option value="{{ $earn->id }}">{{ $earn->name }}</option>
                        @endforeach
                    </select>
                    <label class="focus-label">Earning</label>
                </div>
            </div>
            <div class="col-sm-6 col-md-3">
                        ...
                        <tr>
                            <td>
                                <h2 class="table-avatar">
                                    <a href="{{ url('hr/employee/edit/'. $userEarning->user->id) }}" class="avatar"><img
                                            alt="" src="{{ asset('images/' . $userEarning->user->image) }}"></a>
                                    <a href="{{ url('hr/employee/edit/'. $userEarning->user->id) }}">{{ $userEarning->user->fName }}
                                        {{ $userEarning->user->lName }}
                                        <span>{{ $userEarning->user->position }}</span></a>
                                </h2>
                            </td>
                            <td>{{ $userEarning->user->department }}</td>
                            <td>{{ $userEarning->earningList->name }}</td>
                            <td>{{ $userEarning->earningList->amount }}</td>
                            <td>{{ $userEarning->earningList->type }}</td>
                            <td>
                                @if($userEarning->earningList->inclusion_limit)
                                {{ $userEarning->inclusion_count }} out of
                                {{ $userEarning->earningList->inclusion_limit }}
                                @else
                                -
                                @endif
                            </td>
                            <td>
                                {{ $userEarning->earningList->is_every_payroll ? 'Yes' : 'No' }}

                            </td>
                            <td>
                                <span class="badge {{ $userEarning->active ? 'badge-warning' : 'badge-success' }}">
                                    {{ $userEarning->active ? 'Active' : 'Completed' }}
                                </span>
                            </td>
                            <td>
                                <button class="btn btn-danger delete-earning" data-id="{{ $userEarning->id }}">
...
<!-- Assign Earning -->
<div id="assign_earning" class="modal custom-modal fade" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Assign Earning</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('hr.storeUserEarning') }}" method="POST">
                    @csrf
                    ...
                    <div id="deductions-container">
                        <div class="row row-sm deduction-row">
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Earning</label>
                                    <select name="earning_id[]" class="form-control deduction-select" required>
                                        <option value="">Select Earning</option>
                                        @foreach($earnings as $earn)
                                        <option value="{{ $earn->id }}" data-amount="{{ $earn->amount }}"
                                            data-type="{{ $earn->type }}"
                                            data-inclusion-limit="{{ $earn->inclusion_limit }}">
                                            {{ $earn->name }}
                                        </option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="submit-section">
                        <button type="submit" class="btn btn-primary">Assign Earnings</button>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function createDeductionRow() {
            ...
            <div class="col-sm-2">
                <div class="form-group">
                    <label>Limit</label>
                    <input class="form-control inclusion-limit" type="number" name="inclusion_limit[]" min="1" readonly>
                </div>
            </div>  
            <div class="col-sm-1">
                <div class="form-group">
        ...
        function handleDeductionChange(select) {
            ...
            // Populate the amount and type based on the selected deduction
            if (selectedOption) {
...
<script>
    $(document).ready(function () {
        ...
        $('#department-select').change(function () {
            ...
            if (department) {
                $.ajax({
                    url: '{{ route("HRgetEmployeesByDepartmentEarning") }}',
                    method: 'GET',
                    data: {
    ...
    $(document).ready(function () {
        $('.delete-earning').on('click', function () {
            ...
            if (confirm('Are you sure you want to delete this earning?')) {
                $.ajax({
                    url: '/hr/earning/user/delete/' + earningId,
                    type: 'DELETE',
                    data: {
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            $('button[data-id="' + earningId + '"]').closest('tr')
                                .remove();
...
<script>
    $(document).ready(function () {
        // Handle the change event on the earning selection dropdown
        $(document).on('change', '.deduction-select', function () {
            var $row = $(this).closest('.deduction-row'); // Get the closest row
            var selectedOption = $(this).find('option:selected'); // Get the selected option

            // Get the data attributes from the selected option
            var inclusionLimit = selectedOption.data(
                'inclusion-limit'); // Assuming inclusion limit is set as data attribute

            // If the inclusion limit exists, set it in the inclusion count input
            if (inclusionLimit) {
                $row.find('.inclusion-limit').val(inclusionLimit); // Set the inclusion count input
            } else {
                $row.find('.inclusion-limit').val(''); // Clear the field if no limit
            }

            // Update other fields based on the selected earning if necessary

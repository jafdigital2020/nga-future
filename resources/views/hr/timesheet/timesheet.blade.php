@extends('layouts.hrmaster') @section('title', 'Timesheet') @section('content')
...
<div class="content container-fluid">
    ...
    <!-- Search Filter -->
    <form action="{{ route('attendance.searchr') }}" method="GET">
                        ...
                        <tr>
                            <td>
                                <h2 class="table-avatar">
                                    <a href="{{ url('hr/employee/edit/' . $att->user->id) }}" class="avatar">
                                        @if ($att->user->image)
                                        <img src="{{ asset('images/' . $att->user->image) }}" alt="Profile Image" />
                                        @else
                                        <img src="{{ asset('images/default.png') }}" alt="Profile Image" /></a>
                                    @endif
                                    <a href="{{ url('hr/employee/edit/' . $att->user->id) }}">{{ $att->user->fName }}
                                ...
                                <div class="dropdown action-label">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        ...
                                        <form id="approve-form-{{ $att->id }}"
                                            action="{{ route('att.approvehr', $att->id) }}" method="POST"
                                        ...
                                        <form id="decline-form-{{ $att->id }}"
                                            action="{{ route('att.declinehr', $att->id) }}" method="POST"
                                            style="display:inline;">
                                            @csrf
...
<script>
    ...
    $('.edit-attendance').on('click', function () {
        ...
        $('#editAttendanceForm').attr('action', '/hr/timesheet/approve/' +
    ...
    $('.delete-attendance').on('click', function () {
        ...
        $('#deleteAttendanceForm').attr('action', '/hr/timesheet/delete/' + attId);
